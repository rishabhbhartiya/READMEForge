// src/app/api/card-neobrutalism/route.ts
// ─────────────────────────────────────────────────────────────────────────────
import { NextRequest, NextResponse } from 'next/server'
import { renderNeoBrutalCard, NeoBrutalTheme, NeoBrutalStyle, Dimension } from '@/lib/renderers/card-neobrutalism'
...
export async function GET(req: NextRequest) {
    ...
    const brutalTheme = (p.get('brutalTheme') ?? p.get('theme') ?? 'yellow') as NeoBrutalTheme
    ...
    try {
        ...
        const svg = renderNeoBrutalCard({
            title, value, subtitle, icon,
            brutalTheme,
            pattern: (p.get('pattern') ?? 'dots') as any,
            style: (p.get('style') ?? 'card') as NeoBrutalStyle,
            ...
            shadowOffset: p.get('shadowOffset') ? Number(p.get('shadowOffset')) : undefined,
            borderWidth: p.get('borderWidth') ? Number(p.get('borderWidth')) : undefined,
            depth: p.get('depth') ? Number(p.get('depth')) : undefined,
            rotate: p.get('rotate') ? Number(p.get('rotate')) : undefined,
        ...
        console.error('[card-neobrutalism]', e)
