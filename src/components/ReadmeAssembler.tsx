...
*Generated with [MetalForge](https://metalforge.vercel.app) — Advanced README Component Kit v3*
`
    ...
    <div className="mt-16">
      ...
      <div className="mb-6 flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={copy}
            className={`px-5 py-2 rounded-md text-sm cursor-pointer transition-all font-mono text-[12px]
              ${copied
                ? 'bg-[rgba(57,255,20,0.15)] border border-[rgba(57,255,20,0.4)] text-[#39ff14]'
                : 'btn-chrome'}`}>
            {copied ? '✓ Copied!' : '⚙ Copy Full README'}
          </button>
              ...
              items.map((item, i) => (
                <div key={i} className="flex items-center gap-2 group">
                  <div className="flex-1 font-mono text-[10px] text-[#4a9eff]
                    bg-[rgba(74,158,255,0.06)] border border-[rgba(74,158,255,0.15)]
                    rounded px-3 py-2 truncate min-w-0">
                    {i + 1}. {item.split('?')[0].replace('![', '').split('](')[0]}…
                  </div>
                  {onRemove && (
                    <button
                      onClick={() => onRemove(i)}
                      className="shrink-0 w-6 h-6 rounded text-[#7880a0] hover:text-[#ff4444]
                        hover:bg-[rgba(255,68,68,0.1)] transition-colors text-[12px] cursor-pointer
