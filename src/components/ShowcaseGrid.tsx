...
const SECTIONS = [
  {
    ...
    items: [
      ...
      { name:'terminal-header',        url:'/api/header?name=Alice+Dev&title=Backend+Engineer&tagline=Rust+%C2%B7+Go+%C2%B7+Distributed+Systems&metal=neon&style=terminal&height=260', type:'header' },
  ...
  },
  {
    ...
    items: [
      { name:'neo-commits-dark',     url:'/api/card-neo?title=Commits&value=3%2C291&subtitle=This+year&icon=%E2%97%89&theme=dark&style=raised', type:'neo-card' },
      { name:'neo-stars-warm',       url:'/api/card-neo?title=Stars&value=12.4k&subtitle=Earned&icon=%E2%98%85&theme=warm&style=raised', type:'neo-card' },
      { name:'neo-prs-cool',         url:'/api/card-neo?title=Pull+Requests&value=847&subtitle=Merged&icon=%E2%8E%87&theme=cool&style=floating', type:'neo-card' },
      { name:'neo-pressed-neon',     url:'/api/card-neo?title=Repos&value=58&subtitle=Public&icon=%E2%97%88&theme=neon-dark&style=pressed', type:'neo-card' },
    ]
  },
  {
    label: '// Glassmorphic Cards',
    items: [
      { name:'glass-dark',     url:'/api/card-glass?title=Repositories&value=42&subtitle=Public+repos&icon=%E2%97%88&theme=dark',   type:'glass-card' },
      { name:'glass-aurora',   url:'/api/card-glass?title=Followers&value=5.2k&subtitle=GitHub+followers&icon=%E2%98%85&theme=aurora', type:'glass-card' },
      { name:'glass-sunset',   url:'/api/card-glass?title=Streak&value=89+days&subtitle=Longest+streak&icon=%E2%9A%A1&theme=sunset', type:'glass-card' },
      { name:'glass-ocean',    url:'/api/card-glass?title=Stars&value=12.4k&subtitle=Total+earned&icon=%E2%9C%A6&theme=ocean',  type:'glass-card' },
      { name:'glass-midnight', url:'/api/card-glass?title=Commits&value=3.2k&subtitle=All+time&icon=%E2%97%89&theme=midnight',  type:'glass-card' },
    ]
  },
  {
    ...
    items: [
      ...
      { name:'glitch-neon',         url:'/api/text-anim?text=GLITCH+MODE&effect=glitch&metal=neon&width=500&size=34', type:'text-anim' },
      { name:'neon-flicker',        url:'/api/text-anim?text=NEON+LIGHTS&effect=neon-flicker&metal=electric&width=500&size=32', type:'text-anim' },
  ...
  },
  {
    ...
    items: [
      ...
      { name:'progress-segmented-neon',  url:'/api/progress-bar?label=React&value=85&metal=neon&style=segmented&width=400', type:'progress' },
      { name:'progress-circuit-chrome',  url:'/api/progress-bar?label=Rust&value=60&metal=copper&style=circuit&width=400', type:'progress' },
      { name:'skill-tree-default',       url:'/api/skill-tree?title=Tech+Stack&metal=chrome&width=450', type:'skill-tree' },
      { name:'skill-tree-custom',        url:'/api/skill-tree?title=Languages&skills=TypeScript:92:electric,Python:80:neon,Rust:65:copper,Go:70:titanium&metal=gold&width=450', type:'skill-tree' },
    ]
  },
  {
    label: '// Terminals',
  ...
  },
  {
    ...
    items: [
      ...
      { name:'logo-diamond-rose',     url:'/api/logo-container?text=RS&metal=rose-gold&style=diamond&size=120', type:'logo' },
      { name:'logo-star-neon',        url:'/api/logo-container?text=★&metal=neon&style=star&size=120', type:'logo' },
  ...
  },
  {
    ...
    items: [
      ...
      { name:'gif-neon-frame',        url:'/api/gif-container?frame=neon&metal=neon&width=280&height=200', type:'gif' },
  ...
  },
  {
    ...
    items: [
      ...
      { name:'social-minimal-neon',   url:'/api/social-links?links=github:@dev,twitter:@dev,npm:@dev&metal=neon&style=minimal&width=500', type:'social' },
    ]
  },
]
        ...
        <div key={section.label} className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {section.items.map(item => (
              <div key={item.name} className="metal-card overflow-hidden group transition-all duration-300 hover:-translate-y-1">
                <div className="bg-[#050508] p-4 flex items-center justify-center min-h-[80px] border-b border-[rgba(120,140,200,0.1)]"
                  style={{ backgroundImage:'linear-gradient(rgba(255,255,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.015) 1px,transparent 1px)', backgroundSize:'20px 20px' }}>
                  <img src={item.url} alt={item.name} className="max-w-full max-h-[120px] rounded transition-transform duration-300 group-hover:scale-[1.03]"/>
                </div>
                <div className="p-3 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-mono text-[10px] text-[#7880a0] tracking-[1px] truncate mb-1">{item.name}</p>
                    <span className={`font-mono text-[9px] px-2 py-0.5 rounded border tracking-[1px] uppercase ${TYPE_COLORS[item.type] ?? TYPE_COLORS.banner}`}>
                      {item.type}
                    </span>
                  </div>
                  <button onClick={() => onAdd(`![${item.name}](https://metalforge.vercel.app${item.url})`)}
                    className="btn-ghost px-3 py-1.5 rounded text-[11px] cursor-pointer shrink-0 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    + Add
                  </button>
                </div>
              </div>
