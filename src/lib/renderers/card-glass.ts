...
// Glassmorphism cards — frosted glass effect with SVG blur filters
...
export interface GlassCardOptions {
  ...
  blur?: number     // blur intensity 1-20
}
...
export function renderGlassCard(opts: GlassCardOptions): string {
  ...
  const t = GLASS_THEMES[theme] ?? GLASS_THEMES.dark
  ...
  const uid = `mgls_${Date.now().toString(36)}`

  // Background gradient (behind glass)
  const bgGrad = `
    <radialGradient id="${uid}_bg1" cx="20%" cy="20%" r="80%">
      <stop offset="0%" stop-color="${t.bg3}"/>
      <stop offset="100%" stop-color="${t.bg1}"/>
    </radialGradient>
    <radialGradient id="${uid}_bg2" cx="80%" cy="80%" r="60%">
  ...
  <!-- Background -->
  <rect width="${w}" height="${h}" rx="${r}" fill="url(#${uid}_bg1)"/>
  <!-- Glow blobs -->
  ${glowBlobs}
  <!-- Ambient glow radial -->
  <rect width="${w}" height="${h}" rx="${r}" fill="url(#${uid}_bg2)"/>
  ...
  <rect width="${w}" height="${h}" rx="${r}" fill="url(#${uid}_hi)"/>

  <!-- Outer glass border (refraction effect) -->
  <rect x="0.5" y="0.5" width="${w-1}" height="${h-1}" rx="${r}"
    fill="none" stroke="url(#${uid}_edge)" stroke-width="1.5"/>
